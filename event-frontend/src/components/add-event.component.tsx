...
import DateTimePicker               from "../UI/inputs/date-picker";
...
import {  useEventStore, AddEvent } from "../stores/event.store";

export type AddEventProps = {
  isOpen     : boolean;
  onClose    : () => void;
  initialData: AddEvent;
};

export const AddEventModal :FC<AddEventProps> = ({ initialData, isOpen, onClose }) => {
  ...
  const [eventDate, setEventDate] = useState<Date>(new Date(initialData.eventDate));
    ...
    <div className="fixed inset-0 bg-black/40 backdrop-blur-sm z-50 flex justify-center items-center px-4">
      <div className="bg-white w-full max-w-md rounded-xl shadow-xl p-6 relative">
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-4">
          ...
          <DateTimePicker setDate={setEventDate} value={eventDate}/>
          <button
            type="submit"
